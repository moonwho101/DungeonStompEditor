...
static HRESULT
    CALLBACK
    EnumDisplayModesCallback(LPDDSURFACEDESC pddsd, LPVOID lpContext) {
	/*
...
BOOL D3DAppICheckForPalettized(void) {
	...
	if (bPrimaryPalettized) {
		...
		if (!d3dappi.bFullscreen) {
			for (i = 0; i < 10; i++)
				ppe[i].peFlags = D3DPAL_READONLY;
			for (i = 10; i < 256 - 10; i++)
				ppe[i].peFlags = D3DPAL_FREE | PC_RESERVED;
...
BOOL D3DAppISetDisplayMode(int w, int h, int bpp) {
	d3dappi.ThisMode.w = w;
	d3dappi.ThisMode.h = h;

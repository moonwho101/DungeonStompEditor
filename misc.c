 ...
 */
void D3DAppICopyRectList(int *dstnum, LPD3DRECT dst, int srcnum, LPD3DRECT src) {
	int i;
	for (i = 0; i < srcnum; i++)
		dst[i] = src[i];
	*dstnum = srcnum;
 ...
 */
void D3DAppIMergeRectLists(int *dstnum, LPD3DRECT dst, int src1num, LPD3DRECT src1,
...
void D3DAppISetClientSize(HWND hwnd, int w, int h, BOOL bReturnFromFullscreen) {
	...
	d3dappi.szClient.cx = w;
	d3dappi.szClient.cy = h;
}

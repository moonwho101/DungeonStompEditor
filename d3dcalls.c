...
static HRESULT
    WINAPI
    enumDeviceFunc(LPGUID lpGuid, LPSTR lpDeviceDescription,
...
static HRESULT
    CALLBACK
    EnumTextureFormatsCallback(LPDDSURFACEDESC lpDDSD, LPVOID lpContext) {
	unsigned long m;
	int r, g, b;
...
BOOL D3DAppISetRenderState() {
	...
	pDev->lpVtbl->SetRenderState(pDev, D3DRENDERSTATE_ZENABLE, (DWORD)(d3dapprs.bZBufferOn && d3dappi.ThisDriver.bDoesZBuffer));
	 ...
	 * Set light state
	 */
	pDev->lpVtbl->SetLightState(pDev, D3DLIGHTSTATE_FOGMODE, (DWORD)d3dapprs.bFogEnabled ? d3dapprs.FogMode : D3DFOG_NONE);
